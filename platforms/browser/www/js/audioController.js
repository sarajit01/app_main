app.controller('audioController' , function ($scope , $http , $window , $location , $q , $timeout ) {
    ...
    $scope.playAudio = function () {
        if($scope.currentAudioAlbum !== 'My Favourites') {
            $scope.addToPlayList('https://animusa.net/audio/gallery/add',$scope.currentAudioId,'silent');
        }


        $scope.myAudio = new Audio('https://animusa.net/storage/app/audios/'+$scope.currentAudioSrc);
        console.log('loading');
    ...
    $scope.getMyPlayList = function () {
        ...
        $http.get('https://animusa.net/audio/gallery/get-myplaylist').then(
            function (response) {
                q.resolve(response);
